...
export function DepartmentGridCard({ department: dept, onDelete, onToggleStatus, onViewOrgChart, onClick }: Props) {
  ...
  return (
    <div
      ...
      onClick={(e) => onClick ? onClick(e) : router.push(`/management/departments/${dept.id}`)}
    >
      <div className="h-20 relative flex items-center justify-center" style={{ backgroundColor: generateColor(dept.name) }}>
        {dept.avatarUrl ? (
      ...
      <div className="p-5 flex flex-col gap-3 flex-1">
        ...
        <div className="text-sm">
          {dept.hod ? (
            <div className="flex items-center gap-2">
              <span className="h-6 w-6 rounded-full text-[10px] font-bold flex items-center justify-center text-gray-800 shrink-0"
                style={{ backgroundColor: generateColor(dept.hod.firstName) }}>
        ...
        <div className="flex items-center gap-2 mt-auto">
          ...
          <div className="ml-auto">
            <button ref={btnRef} type="button" onClick={(e) => { e.stopPropagation(); setMenuOpen(!menuOpen) }}
            ...
            {menuOpen && (
              <>
                ...
                <div className="fixed z-50 w-44 rounded-lg border bg-popover shadow-md py-1"
                  style={(() => {
        ...
        {dept.subjectNames.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {visibleSubjects.map((sub) => (
              <span key={sub} className="text-xs px-2 py-0.5 rounded-full bg-muted text-foreground">
                {sub}
              </span>
            ))}
            {extraSubjects > 0 && (
